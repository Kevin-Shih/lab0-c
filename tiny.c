...
{
    printf("\r%s:%d %d - '%s'", inet_ntoa(c_addr->sin_addr),
...
void parse_request(int fd, struct sockaddr_in *clientaddr, http_request *req)
{
    ...
    while (buf[0] != '\n' && buf[1] != '\n') { /* \n || \r\n */
        ...
        if (buf[0] == 'R' && buf[1] == 'a' && buf[2] == 'n') {
            sscanf(buf, "Range: bytes=%64lu-%64lu",
                   (unsigned long *) &req->offset, (unsigned long *) &req->end);
            // Range: [start, end]
            if (req->end != 0) {
                req->end++;
            }
    ...
    while ((p = strchr(p, '/')))
        *p++ = ' ';
    strncpy(req->filename, buf, strlen(buf) + 1);
#ifdef LOG_ACCESS
